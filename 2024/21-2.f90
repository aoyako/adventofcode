program twentyone_two
    implicit none
    integer, parameter :: int128 = selected_int_kind(38)
    integer :: ios
    integer(int128) :: ncode, x, y, i, result, tmp
    character(256) :: line
    ...
    do
        ...
        tmp = res(line(1:1), 1_int128, "A", 26_int128)
        do i = 2, len_trim(line)
            tmp = tmp + res(line(i:i), 1_int128, line(i-1:i-1), 26_int128)
            y = 0
            read(line(i-1:i-1), *) y
            x = x*10 + y
        end do
...
recursive function res(end, press, begin, depth) result(output)
    ...
    integer(int128), intent(in) :: press, depth
    integer(int128) :: output
    integer(int128) :: cooords_dir(10, 10), empty_x, empty_y, i, j, legs(120, 120, 120), nx, nlayers
    integer(int128) :: bx, by, ex, ey, dx, dy, scale, yscale, length, xscale
...
subroutine set_coords(dir, x, y)
    integer(int128), intent(inout) :: x, y
